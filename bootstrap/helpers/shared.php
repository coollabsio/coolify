...
/**
 * Fixed path of the optional self-hosted custom stylesheet.
 * Bind-mounted from /data/coolify/themes in production.
 */
...
/** Built-in theme presets shipped in resources/themes, slug => label. */
...
/** CSS chosen under Settings > Theme (preset file or pasted custom CSS). Empty string when unset. */
function instanceThemeCss(?InstanceSettings $settings): string
{
    ...
    // ponytail: rendered inline in <style>, so the only thing that must never appear is a closing tag
    return str_ireplace('</style', '<\/style', $css);
}

/**
 * Convert an iTerm2 .itermcolors plist into a Coolify theme (html.dark scope).
 * Returns null when the input is not an iTerm2 colour scheme.
 */
function itermColorsToCss(string $plist): ?string
{
    ...
    return "/* Converted from an iTerm2 colour scheme. Edit freely. */\n".themeCssFromPalette([
...
/** Same variable set as resources/themes/*.css. Keys: app panel surface raised selected fg fg_dim fg_faint accent accent_fg success error. */
