...
function dockerComposeParserForApplications(Application $application, Collection $compose): Collection
{
    ...
    $fileStorages = $application->fileStorages();
    ...
    foreach ($services as $serviceName => $service) {
        ...
        // TODO: refactor generateApplicationContainerName
        ...
        if ($volumes->count() > 0) {
            foreach ($volumes as $index => $volume) {
                ...
                // dispatch(new ServerFilesFromServerJob($application));
        ...
        if (data_get($application, 'settings.connect_to_docker_network')) {
        ...
        foreach ($environment as $key => $value) {
            ...
            if ($key->startsWith('SERVICE_FQDN') || $value->startsWith('$SERVICE_FQDN') || $value->startsWith('${SERVICE_FQDN')) {
                ...
                $application->environment_variables()->where('key', $keyUrl)->where('application_id', $application->id)->firstOrCreate([
                    'key' => $keyUrl,
                    'application_id' => $application->id,
                ], [
                    'value' => $url,
