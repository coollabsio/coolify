...
function connectProxyToNetworks(Server $server)
{
    ...
    if ($server->isSwarm()) {
        $commands = $networks->map(function ($network) {
            ...
            return [
                "docker network ls --format '{{.Name}}' | grep '^{$network}$' >/dev/null || docker network create --driver overlay --attachable --ipv6 {$safe} >/dev/null",
        ...
        $commands = $networks->map(function ($network) {
            $safe = escapeshellarg($network);
            return [
                "docker network ls --format '{{.Name}}' | grep '^{$network}$' >/dev/null || docker network create --attachable --ipv6 {$safe} >/dev/null",
                "docker network connect {$safe} coolify-proxy >/dev/null 2>&1 || true",
                "echo 'Successfully connected coolify-proxy to {$safe} network.'",
            ];
        });
    ...
    if ($server->isSwarm()) {
        $commands = $networks->map(function ($network) {
            ...
            return [
                ...
                "docker network ls --format '{{.Name}}' | grep -q '^{$network}$' || docker network create --driver overlay --attachable --ipv6 {$safe}",
            ];
        });
    } else {
        $commands = $networks->map(function ($network) {
            ...
            return [
                "echo 'Ensuring network {$safe} exists...'",
                "docker network ls --format '{{.Name}}' | grep -q '^{$network}$' || docker network create --attachable --ipv6 {$safe}",
            ];
        });
    }
