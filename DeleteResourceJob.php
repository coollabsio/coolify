...
class DeleteResourceJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    private function stopPreviewContainers(array $containers, $server, int $timeout = 30)
    {
        ...
        foreach ($containers as $container) {
            ...
            // Check if container exists before adding to the list
            $checkCmd = "docker ps -a --format '{{.Names}}' | grep -w {$containerName}";
            $result = instant_remote_process([$checkCmd], $server, throwError: false);
            if (trim($result[0] ?? '') !== '') {
                $existingContainerNames[] = escapeshellarg($containerName);
            }
        }

        if (empty($existingContainerNames)) {
            return;
        }

        $containerList = implode(' ', $existingContainerNames);
        $commands = [
