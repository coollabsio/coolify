...
class ApplicationDeploymentJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    private function graceful_shutdown_contain(string $containerName)
    {
        try {
            $timeout = isDev() ? 1 : 30;
            ...
            if (trim($result[0] ?? '') !== '') {
                ...
                $this->application_deployment_queue->addLogEntry("Container $containerName does not exist, skipping stop/rm.", 'stderr');
            }
