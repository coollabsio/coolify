...
class ApplicationDeploymentJobNew implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $this->execute_remote_command(
                [
                    "command" => "docker rm -f {$this->deployment_uuid} >/dev/null 2>&1",
                    "hidden" => true,
                ]
            );
        }
    ...
    private function deploy()
    {
        ...
        if (!$this->force_rebuild) {
            ...
            if (Str::of($this->saved_outputs->get('local_image_found'))) {
                $this->execute_remote_command([
                    "echo 'Docker Image found locally with the same Git Commit SHA. Build step skipped...'"
                ]);
...
COPY --from=$this->build_image_name /app/{$this->application->publish_directory} .";
        ...
        if ($this->application->deploymentType() === 'source') {
            ...
            if ($this->source->getMorphClass() == 'App\Models\GithubApp') {
                if ($this->source->is_public) {
                    ...
                    $commands = $this->execute_in_builder("git clone -q -b {$this->application->git_branch} $source_html_url_scheme://x-access-token:$github_access_token@$source_html_url_host/{$this->application->git_repository}.git {$this->workdir}");
                    if ($this->pull_request_id !== 0) {
    ...
    private function clone_repository()
    {
        $this->execute_remote_command(
            ...
            [
                "command" => $this->execute_in_builder("cd {$this->workdir} && git rev-parse HEAD"),
                "hidden" => true,
                "save" => "git_commit_sha"
            ],
        );
