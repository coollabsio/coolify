...
class ApplicationDeploymentJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    public function tags()
    {
        ...
        return [
            "App\Models\ApplicationDeploymentQueue:" .
            $this->application_deployment_queue_id,
        ];
    }
    ...
    public function __construct(public int $application_deployment_queue_id)
    {
        ...
        $this->application_deployment_queue = ApplicationDeploymentQueue::find(
            $this->application_deployment_queue_id
        );
        $this->nixpacks_plan_json = collect([]);

        $this->application = Application::find(
            $this->application_deployment_queue->application_id
        );
        ...
        $this->pull_request_id =
            $this->application_deployment_queue->pull_request_id;
        $this->commit = $this->application_deployment_queue->commit;
        $this->rollback = $this->application_deployment_queue->rollback;
        $this->disableBuildCache =
            $this->application->settings->disable_build_cache;
        $this->force_rebuild =
            $this->application_deployment_queue->force_rebuild;
        ...
        $this->only_this_server =
            $this->application_deployment_queue->only_this_server;

        $this->git_type = data_get(
        ...
        if ($source) {
            $this->source = $source
                ->getMorphClass()
                ::where("id", $this->application->source->id)
                ->first();
        }
        $this->server = Server::find(
            $this->application_deployment_queue->server_id
        );
        $this->timeout = $this->server->settings->dynamic_timeout;
        $this->destination = $this->server
            ->destinations()
            ->where("id", $this->application_deployment_queue->destination_id)
            ->first();
        ...
        $this->is_this_additional_server =
            $this->application
                ...
                ->count() > 0;
        $this->preserveRepository =
            $this->application->settings->is_preserve_repository_enabled;

        $this->basedir = $this->application->generateBaseDir(
            $this->deployment_uuid
        );
        $this->workdir =
            "{$this->basedir}" . rtrim($this->application->base_directory, "/");
        $this->configuration_dir =
            application_configuration_dir() . "/{$this->application->uuid}";
        $this->is_debug_enabled =
            $this->application->settings->is_debug_enabled;

        $this->container_name = generateApplicationContainerName(
            $this->application,
            $this->pull_request_id
        );
        if (
            $this->application->settings->custom_internal_name &&
            !$this->application->settings->is_consistent_container_name_enabled
        ) {
        ...
        // Set preview fqdn
        if ($this->pull_request_id !== 0) {
            $this->preview = $this->application->generate_preview_fqdn(
                $this->pull_request_id
            );
            ...
            if ($this->application->build_pack === "dockerfile") {
                if (data_get($this->application, "dockerfile_location")) {
                    $this->dockerfile_location =
                        $this->application->dockerfile_location;
                }
            }
        }
    }
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $allContainers = instant_remote_process(
                [
                    "docker network inspect {$this->destination->network} -f '{{json .Containers}}' ",
                ],
                $this->server
            );

            if (!is_null($allContainers)) {
                $allContainers = format_docker_command_output_to_json(
                    $allContainers
                );
                $ips = collect([]);
                if (count($allContainers) > 0) {
                    $allContainers = $allContainers[0];
                    $allContainers = collect($allContainers)
                        ->sort()
                        ->values();
            ...
            if (
                data_get($this->application, "settings.is_build_server_enabled")
            ) {
                $teamId = data_get(
                    $this->application,
                ...
                if ($buildServers->count() === 0) {
                    ...
                    $this->application_deployment_queue->build_server_id =
                        $this->build_server->id;
                    $this->application_deployment_queue->addLogEntry(
                        "Found a suitable build server ({$this->build_server->name})."
                    );
                    $this->original_server = $this->server;
            ...
            if (
                ...
                $this->application->is_github_based()
            ) {
                ApplicationPullRequestUpdateJob::dispatch(
                    application: $this->application,
                    preview: $this->preview,
            ...
            $this->application_deployment_queue->addLogEntry(
                "Gracefully shutting down build container: {$this->deployment_uuid}"
            );
            $this->graceful_shutdown_container($this->deployment_uuid);

            ServiceStatusChanged::dispatch(
                data_get($this->application, "environment.project.team.id")
            );
        }
    }
    ...
    private function deploy_simple_dockerfile()
    {
        ...
        $this->application_deployment_queue->addLogEntry(
            "Starting deployment of {$this->application->name} to {$this->server->name}."
        );
        $this->prepare_builder_image();
        $this->execute_remote_command([
            executeInDocker(
                $this->deployment_uuid,
                "echo '$dockerfile_base64' | base64 -d | tee {$this->workdir}{$this->dockerfile_location} > /dev/null"
            ),
        ]);
    ...
    private function deploy_dockerimage_buildpack()
    {
        ...
        if (str($this->application->docker_registry_image_tag)->isEmpty()) {
            ...
            $this->dockerImageTag =
                $this->application->docker_registry_image_tag;
        }
        $this->application_deployment_queue->addLogEntry(
            "Starting deployment of {$this->dockerImage}:{$this->dockerImageTag} to {$this->server->name}."
        );
        $this->generate_image_names();
        $this->prepare_builder_image();
        $this->generate_compose_file();
    ...
    private function deploy_docker_compose_buildpack()
    {
        if (data_get($this->application, "docker_compose_location")) {
            $this->docker_compose_location =
                $this->application->docker_compose_location;
        }
        if (
            data_get($this->application, "docker_compose_custom_start_command")
        ) {
            $this->docker_compose_custom_start_command =
                $this->application->docker_compose_custom_start_command;
            if (
                !str($this->docker_compose_custom_start_command)->contains(
                    "--project-directory"
                )
            ) {
                $this->docker_compose_custom_start_command = str(
                    $this->docker_compose_custom_start_command
                )
                    ->replaceFirst(
                        "compose",
        ...
        if (
            data_get($this->application, "docker_compose_custom_build_command")
        ) {
            $this->docker_compose_custom_build_command =
                $this->application->docker_compose_custom_build_command;
            if (
                !str($this->docker_compose_custom_build_command)->contains(
                    "--project-directory"
                )
            ) {
                $this->docker_compose_custom_build_command = str(
                    $this->docker_compose_custom_build_command
                )
                    ->replaceFirst(
                        "compose",
        ...
        if ($this->pull_request_id === 0) {
            ...
            $this->application_deployment_queue->addLogEntry(
                "Starting pull request (#{$this->pull_request_id}) deployment of {$this->customRepository}:{$this->application->git_branch} to {$this->server->name}."
            );
        }
        $this->prepare_builder_image();
        ...
        if ($this->preserveRepository) {
            foreach ($this->application->fileStorages as $fileStorage) {
                ...
                $realPathInGit = str($path)
                    ->replace($this->application->workdir(), $this->workdir)
                    ->value();
                // check if the file is a directory or a file inside the repository
                $this->execute_remote_command([
                    executeInDocker(
                        $this->deployment_uuid,
                        "stat -c '%F' {$realPathInGit}"
                    ),
                    "hidden" => true,
                    "ignore_errors" => true,
                    "save" => $saveName,
                ...
                if ($this->saved_outputs->has($saveName)) {
                    ...
                    if (
                        $fileStat->value() === "directory" &&
                        !$fileStorage->is_directory
                    ) {
        ...
        if ($this->docker_compose_custom_build_command) {
            $this->execute_remote_command([
                executeInDocker(
                    $this->deployment_uuid,
                    "cd {$this->basedir} && {$this->docker_compose_custom_build_command}"
                ),
                "hidden" => true,
            ]);
        } else {
        ...
        if ($this->application->settings->is_raw_compose_deployment_enabled) {
            if ($this->docker_compose_custom_start_command) {
                ...
                $this->execute_remote_command([
                    executeInDocker(
                        $this->deployment_uuid,
                        "cd {$this->workdir} && {$this->docker_compose_custom_start_command}"
                    ),
                    "hidden" => true,
                ]);
            } else {
            ...
            if ($this->docker_compose_custom_start_command) {
                ...
                $this->execute_remote_command([
                    executeInDocker(
                        $this->deployment_uuid,
                        "cd {$this->basedir} && {$this->docker_compose_custom_start_command}"
                    ),
                    "hidden" => true,
                ]);
            } else {
    ...
    {
        $this->application_deployment_queue->addLogEntry(
            "Starting deployment of {$this->customRepository}:{$this->application->git_branch} to {$this->server->name}."
        );
        if ($this->use_build_server) {
            $this->server = $this->build_server;
        }
    ...
    private function write_deployment_configurations()
    {
        ...
        if (isset($this->docker_compose_base64)) {
            ...
            $readme = generate_readme_file(
                $this->application->name,
                $this->application_deployment_queue->updated_at
            );
    ...
    private function push_to_docker_registry()
    {
        ...
        try {
            instant_remote_process(
                [
                    "docker images --format '{{json .}}' {$this->production_image_name}",
                ],
                $this->server
            );
            $this->application_deployment_queue->addLogEntry(
            ...
            $this->application_deployment_queue->addLogEntry(
                "Pushing image to docker registry ({$this->production_image_name})."
            );
            $this->execute_remote_command([
                executeInDocker(
                    $this->deployment_uuid,
                    "docker push {$this->production_image_name}"
                ),
                "hidden" => true,
            ]);
            if ($this->application->docker_registry_image_tag) {
                ...
                $this->application_deployment_queue->addLogEntry(
                    "Tagging and pushing image with {$this->application->docker_registry_image_tag} tag."
                );
                $this->execute_remote_command(
                    [
                        executeInDocker(
                            $this->deployment_uuid,
                            "docker tag {$this->production_image_name} {$this->application->docker_registry_image_name}:{$this->application->docker_registry_image_tag}"
                        ),
                        "ignore_errors" => true,
                        "hidden" => true,
                    ],
                    [
                        executeInDocker(
                            $this->deployment_uuid,
                            "docker push {$this->application->docker_registry_image_name}:{$this->application->docker_registry_image_tag}"
                        ),
                        "ignore_errors" => true,
                        "hidden" => true,
                    ]
    ...
    {
        $this->application_deployment_queue->addLogEntry(
            "Restarting {$this->customRepository}:{$this->application->git_branch} on {$this->server->name}."
        );
        $this->prepare_builder_image();
        $this->check_git_if_build_needed();
        $this->generate_image_names();
    ...
    private function should_skip_build()
    {
        if (str($this->saved_outputs->get("local_image_found"))->isNotEmpty()) {
            if ($this->is_this_additional_server) {
                $this->application_deployment_queue->addLogEntry(
                    "Image found ({$this->production_image_name}) with the same Git Commit SHA. Build step skipped."
                );
                $this->generate_compose_file();
                $this->push_to_docker_registry();
                $this->rolling_update();
            ...
            if (!$this->application->isConfigurationChanged()) {
                $this->application_deployment_queue->addLogEntry(
                    "No configuration changed & image found ({$this->production_image_name}) with the same Git Commit SHA. Build step skipped."
                );
                $this->generate_compose_file();
                $this->push_to_docker_registry();
                $this->rolling_update();
            ...
            $this->application_deployment_queue->addLogEntry(
                "Image not found ({$this->production_image_name}). Building new image."
            );
        }
        if ($this->restart_only) {
    ...
    private function check_image_locally_or_remotely()
    {
        ...
        if (
            str($this->saved_outputs->get("local_image_found"))->isEmpty() &&
    ...
    private function save_environment_variables()
    {
        ...
        if ($sort) {
            $sorted_environment_variables = $this->application->environment_variables->sortBy(
                "key"
            );
            $sorted_environment_variables_preview = $this->application->environment_variables_preview->sortBy(
                "key"
            );
        } else {
            $sorted_environment_variables = $this->application->environment_variables->sortBy(
        ...
        if ($this->build_pack === "dockercompose") {
            $sorted_environment_variables = $sorted_environment_variables->filter(
                function ($env) {
                    return !str($env->key)->startsWith("SERVICE_FQDN_") &&
                        !str($env->key)->startsWith("SERVICE_URL_");
                }
            );
            $sorted_environment_variables_preview = $sorted_environment_variables_preview->filter(
                function ($env) {
                    return !str($env->key)->startsWith("SERVICE_FQDN_") &&
                        !str($env->key)->startsWith("SERVICE_URL_");
                }
        ...
        if ($this->pull_request_id === 0) {
            ...
            if ($this->build_pack === "dockercompose") {
                $domains =
                    collect(
                        json_decode($this->application->docker_compose_domains)
                    ) ?? collect([]);
                ...
                foreach ($domains as $forServiceName => $domain) {
                    ...
                    if (filled($parsedDomain)) {
                        ...
                        $coolifyUrl = $coolifyUrl
                            ->withScheme($coolifyScheme)
                            ->withHost($coolifyFqdn)
                            ->withPort(null);
                        $envs->push(
                            "SERVICE_URL_" .
                                str($forServiceName)->upper() .
                                "=" .
                                $coolifyUrl->__toString()
                        );
                        $envs->push(
                            "SERVICE_FQDN_" .
                                str($forServiceName)->upper() .
        ...
        if ($envs->isEmpty()) {
            ...
            $this->execute_remote_command([
                executeInDocker(
                    $this->deployment_uuid,
                    "echo '$envs_base64' | base64 -d | tee $this->workdir/{$this->env_filename} > /dev/null"
                ),
            ]);
            if ($this->use_build_server) {
                $this->server = $this->original_server;
    ...
    private function elixir_finetunes()
    {
        ...
        $mix_env = $this->application->{$envType}
            ->where("key", "MIX_ENV")
            ->first();
        if ($mix_env) {
            if ($mix_env->is_build_time === false) {
                $this->application_deployment_queue->addLogEntry(
                    "MIX_ENV environment variable is not set as build time.",
                    type: "error"
                );
                $this->application_deployment_queue->addLogEntry(
                    "Please set MIX_ENV environment variable to be build time variable if you facing any issues with the deployment.",
                    type: "error"
                );
            }
        } else {
            $this->application_deployment_queue->addLogEntry(
                "MIX_ENV environment variable not found.",
                type: "error"
            );
            $this->application_deployment_queue->addLogEntry(
                "Please add MIX_ENV environment variable and set it to be build time variable if you facing any issues with the deployment.",
                type: "error"
            );
        }
        $secret_key_base = $this->application->{$envType}
            ->where("key", "SECRET_KEY_BASE")
            ->first();
        if ($secret_key_base) {
            if ($secret_key_base->is_build_time === false) {
                $this->application_deployment_queue->addLogEntry(
                    "SECRET_KEY_BASE environment variable is not set as build time.",
                    type: "error"
                );
                $this->application_deployment_queue->addLogEntry(
                    "Please set SECRET_KEY_BASE environment variable to be build time variable if you facing any issues with the deployment.",
                    type: "error"
                );
            }
        } else {
            $this->application_deployment_queue->addLogEntry(
                "SECRET_KEY_BASE environment variable not found.",
                type: "error"
            );
            $this->application_deployment_queue->addLogEntry(
                "Please add SECRET_KEY_BASE environment variable and set it to be build time variable if you facing any issues with the deployment.",
                type: "error"
            );
        }
        $database_url = $this->application->{$envType}
            ->where("key", "DATABASE_URL")
            ->first();
        if ($database_url) {
            if ($database_url->is_build_time === false) {
                $this->application_deployment_queue->addLogEntry(
                    "DATABASE_URL environment variable is not set as build time.",
                    type: "error"
                );
                $this->application_deployment_queue->addLogEntry(
                    "Please set DATABASE_URL environment variable to be build time variable if you facing any issues with the deployment.",
                    type: "error"
                );
            }
        } else {
            ...
            $this->application_deployment_queue->addLogEntry(
                "Please add DATABASE_URL environment variable and set it to be build time variable if you facing any issues with the deployment.",
                type: "error"
            );
        }
    }
    ...
    private function laravel_finetunes()
    {
        ...
        $nixpacks_php_fallback_path = $this->application->{$envType}
            ->where("key", "NIXPACKS_PHP_FALLBACK_PATH")
            ->first();
        $nixpacks_php_root_dir = $this->application->{$envType}
            ->where("key", "NIXPACKS_PHP_ROOT_DIR")
            ->first();
        ...
        if (!$nixpacks_php_fallback_path) {
            ...
            $nixpacks_php_fallback_path->resourceable_id =
                $this->application->id;
            $nixpacks_php_fallback_path->resourceable_type =
        ...
        if (!$nixpacks_php_root_dir) {
            ...
            $nixpacks_php_root_dir->resourceable_type =
                "App\Models\Application";
    ...
    private function rolling_update()
    {
        if ($this->server->isSwarm()) {
            ...
            $this->execute_remote_command([
                executeInDocker(
                    $this->deployment_uuid,
                    "docker stack deploy --detach=true --with-registry-auth -c {$this->workdir}{$this->docker_compose_location} {$this->application->uuid}"
                ),
            ]);
            $this->application_deployment_queue->addLogEntry(
            ...
            if (
                count($this->application->ports_mappings_array) > 0 ||
                (bool) $this->application->settings
                    ->is_consistent_container_name_enabled ||
                str(
                    $this->application->settings->custom_internal_name
                )->isNotEmpty() ||
                $this->pull_request_id !== 0 ||
                str($this->application->custom_docker_run_options)->contains(
                    "--ip"
                ) ||
                str($this->application->custom_docker_run_options)->contains(
                    "--ip6"
                ...
                if (
                    ...
                    $this->application_deployment_queue->addLogEntry(
                        "Custom internal name is set, rolling update is not supported."
                    );
                }
                if ($this->pull_request_id !== 0) {
                    $this->application->settings->is_consistent_container_name_enabled = true;
                    $this->application_deployment_queue->addLogEntry(
                        "Pull request deployment, rolling update is not supported."
                    );
                }
                if (
                    str(
                        $this->application->custom_docker_run_options
                    )->contains("--ip") ||
                    str(
                        $this->application->custom_docker_run_options
                    )->contains("--ip6")
                    ...
                    $this->application_deployment_queue->addLogEntry(
                        "Custom IP address is set, rolling update is not supported."
                    );
                }
                $this->stop_running_container(force: true);
    ...
    private function health_check()
    {
        if ($this->server->isSwarm()) {
            ...
            if (
                $this->application->isHealthcheckDisabled() &&
                $this->application->custom_healthcheck_found === false
            ) {
            ...
            if ($this->container_name) {
                ...
                if (
                    $this->full_healthcheck_url &&
                    !$this->application->custom_healthcheck_found
                ) {
                    $this->application_deployment_queue->addLogEntry(
                        "Healthcheck URL (inside the container): {$this->full_healthcheck_url}"
                    );
                }
                $this->application_deployment_queue->addLogEntry(
                    "Waiting for the start period ({$this->application->health_check_start_period} seconds) before starting healthcheck."
                );
                $sleeptime = 0;
                while (
                    $sleeptime < $this->application->health_check_start_period
                ) {
                    Sleep::for(1)->seconds();
                    $sleeptime++;
                }
                while ($counter <= $this->application->health_check_retries) {
                    ...
                    $this->application_deployment_queue->addLogEntry(
                        "Attempt {$counter} of {$this->application->health_check_retries} | Healthcheck status: {$this->saved_outputs->get(
                            "health_check"
                        )}"
                    );
                    $health_check_logs = data_get(
                        collect(
                            json_decode(
                                $this->saved_outputs->get("health_check_logs")
                            )
                        )->last(),
                        "Output",
                        "(no logs)"
                    ...
                    $health_check_return_code = data_get(
                        collect(
                            json_decode(
                                $this->saved_outputs->get("health_check_logs")
                            )
                    ...
                    if (
                        $health_check_logs !== "(no logs)" ||
                        $health_check_return_code !== "(no return code)"
                    ) {
                        $this->application_deployment_queue->addLogEntry(
                            "Healthcheck logs: {$health_check_logs} | Return code: {$health_check_return_code}"
                        );
                    }

                    if (
                        str($this->saved_outputs->get("health_check"))
                            ->replace('"', "")
                            ->value() === "healthy"
                    ) {
                        $this->newVersionIsHealthy = true;
                    ...
                    $sleeptime = 0;
                    while (
                        $sleeptime < $this->application->health_check_interval
                    ) {
                        Sleep::for(1)->seconds();
                        $sleeptime++;
                    }
    ...
    private function deploy_pull_request()
    {
        ...
        $this->generate_image_names();
        $this->application_deployment_queue->addLogEntry(
            "Starting pull request (#{$this->pull_request_id}) deployment of {$this->customRepository}:{$this->application->git_branch}."
        );
        $this->prepare_builder_image();
        $this->check_git_if_build_needed();
        $this->clone_repository();
    ...
    private function prepare_builder_image()
    {
        ...
        $this->serverUserHomeDir = instant_remote_process(
            ['echo $HOME'],
            $this->server
        );
        ...
        if ($this->use_build_server) {
            if ($this->dockerConfigFileExists === "NOK") {
                throw new RuntimeException(
                    'Docker config file (~/.docker/config.json) not found on the build server. Please run "docker login" to login to the docker registry on the server.'
                );
            }
        ...
        $this->application_deployment_queue->addLogEntry(
            "Preparing container with helper image: $helperImage."
        );
        $this->graceful_shutdown_container($this->deployment_uuid);
        $this->execute_remote_command(
    ...
    private function deploy_to_additional_destinations()
    {
        ...
        if ($this->server->isSwarm()) {
            $this->application_deployment_queue->addLogEntry(
                "Additional destinations are not supported in swarm mode."
            );
        ...
        foreach ($destination_ids as $destination_id) {
            ...
            if ($server->team_id !== $this->mainServer->team_id) {
                $this->application_deployment_queue->addLogEntry(
                    "Skipping deployment to {$server->name}. Not in the same team?!"
                );
            ...
            $this->application_deployment_queue->addLogEntry(
                "Deployment to {$server->name}. Logs: " .
                    route("project.application.deployment.show", [
                        "project_uuid" => data_get(
                            $this->application,
                            "environment.project.uuid"
                        ),
                        "application_uuid" => data_get(
                            $this->application,
                            "uuid"
                        ),
                        "deployment_uuid" => $deployment_uuid,
    ...
    private function check_git_if_build_needed()
    {
        if (
            ...
            $this->source->getMorphClass() === \App\Models\GithubApp::class &&
            $this->source->is_public === false
        ) {
            $repository = githubApi(
                $this->source,
                "repos/{$this->customRepository}"
            );
            $data = data_get($repository, "data");
            $repository_project_id = data_get($data, "id");
            if (isset($repository_project_id)) {
                if (
                    blank($this->application->repository_project_id) ||
                    $this->application->repository_project_id !==
        ...
        if ($private_key) {
            ...
            $this->execute_remote_command(
                ...
                [
                    executeInDocker(
                        $this->deployment_uuid,
                        "GIT_SSH_COMMAND=\"ssh -o ConnectTimeout=30 -p {$this->customPort} -o Port={$this->customPort} -o LogLevel=ERROR -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null\" git ls-remote {$this->fullRepoUrl} {$local_branch}"
                    ),
                    "hidden" => true,
                    "save" => "git_commit_sha",
                ]
        ...
        if ($this->saved_outputs->get("git_commit_sha") && !$this->rollback) {
            $this->commit = $this->saved_outputs
                ->get("git_commit_sha")
                ->before("\t");
    ...
    private function clone_repository()
    {
        ...
        $this->application_deployment_queue->addLogEntry(
            "Importing {$this->customRepository}:{$this->application->git_branch} (commit sha {$this->application->git_commit_sha}) to {$this->basedir}."
        );
        if ($this->pull_request_id !== 0) {
            $this->application_deployment_queue->addLogEntry(
                "Checking out tag pull/{$this->pull_request_id}/head."
            );
        }
        $this->execute_remote_command([$importCommands, "hidden" => true]);
        $this->create_workdir();
        $this->execute_remote_command([
            executeInDocker(
                $this->deployment_uuid,
                "cd {$this->workdir} && git log -1 {$this->commit} --pretty=%B"
            ),
            "hidden" => true,
            "save" => "commit_message",
        ]);
        if ($this->saved_outputs->get("commit_message")) {
            ...
            ApplicationDeploymentQueue::whereCommit($this->commit)
                ->whereApplicationId($this->application->id)
                ->update(["commit_message" => $commit_message->value()]);
        }
    }
    ...
    private function generate_git_import_commands()
    {
        [
            "commands" => $commands,
            "branch" => $this->branch,
            "fullRepoUrl" => $this->fullRepoUrl,
        ] = $this->application->generateGitImportCommands(
            deployment_uuid: $this->deployment_uuid,
            pull_request_id: $this->pull_request_id,
    ...
    private function generate_nixpacks_confs()
    {
        ...
        $this->application_deployment_queue->addLogEntry(
            "Generating nixpacks configuration with: $nixpacks_command"
        );
        $this->execute_remote_command(
            [
        ...
        if ($this->saved_outputs->get("nixpacks_type")) {
            ...
            if (str($this->nixpacks_type)->isEmpty()) {
                throw new RuntimeException(
                    "Nixpacks failed to detect the application type. Please check the documentation of Nixpacks: https://nixpacks.com/docs/providers"
                );
            }
        }
        ...
        if ($this->saved_outputs->get("nixpacks_plan")) {
            ...
            if ($this->nixpacks_plan) {
                $this->application_deployment_queue->addLogEntry(
                    "Found application type: {$this->nixpacks_type}."
                );
                $this->application_deployment_queue->addLogEntry(
                    "If you need further customization, please check the documentation of Nixpacks: https://nixpacks.com/docs/providers/{$this->nixpacks_type}"
                );
                ...
                if ($is_laravel) {
                    ...
                    data_set(
                        $parsed,
                        "variables.NIXPACKS_PHP_FALLBACK_PATH",
                        $variables[0]->value
                    );
                    data_set(
                        $parsed,
                        "variables.NIXPACKS_PHP_ROOT_DIR",
                        $variables[1]->value
                    );
                }
                ...
                $this->application_deployment_queue->addLogEntry(
                    "Final Nixpacks plan: {$this->nixpacks_plan}",
                    hidden: true
                );
    ...
    private function generate_nixpacks_env_variables()
    {
        ...
        if ($this->pull_request_id === 0) {
            foreach (
                $this->application->nixpacks_environment_variables
                as $env
            ) {
                if (!is_null($env->real_value)) {
                    $this->env_nixpacks_args->push(
                        "--env {$env->key}={$env->real_value}"
                    );
                }
            }
        } else {
            foreach (
                $this->application->nixpacks_environment_variables_preview
                as $env
            ) {
                if (!is_null($env->real_value)) {
                    $this->env_nixpacks_args->push(
    ...
    private function generate_coolify_env_variables(): Collection
    {
        ...
        if ($this->pull_request_id !== 0) {
            ...
            if (
                ...
                $this->application->compose_parsing_version === "1" ||
                $this->application->compose_parsing_version === "2"
            ) {
                if (
                    $this->application->environment_variables_preview
                        ->where("key", "COOLIFY_BRANCH")
                        ->isEmpty()
                ) {
                ...
                if (
                    $this->application->environment_variables_preview
                        ->where("key", "COOLIFY_RESOURCE_UUID")
                        ->isEmpty()
                ) {
                    $coolify_envs->put(
                ...
                if (
                    $this->application->environment_variables_preview
                        ->where("key", "COOLIFY_CONTAINER_NAME")
                        ->isEmpty()
                ) {
                    $coolify_envs->put(
    ...
    private function generate_env_variables()
    {
        ...
        if ($this->pull_request_id === 0) {
                ...
                if (!is_null($env->real_value)) {
                    ...
                    if (str($env->real_value)->startsWith('$')) {
                        ...
                        if ($variable_key->startsWith("COOLIFY_")) {
                            $variable = $coolify_envs->get(
                                $variable_key->value()
                            );
                            if (filled($variable)) {
                                $this->env_args->prepend(
                                    $variable,
                                    $variable_key->value()
                                );
                            }
            ...
            foreach (
                $this->application->build_environment_variables_preview
                as $env
            ) {
                if (!is_null($env->real_value)) {
                    $this->env_args->put($env->key, $env->real_value);
    ...
    private function generate_compose_file()
    {
        ...
        if (data_get($this->application, "custom_labels")) {
            ...
            $labels = collect(
                preg_split(
                    "/\r\n|\n|\r/",
                    base64_decode($this->application->custom_labels)
                )
            )->filter(fn($value) => !Str::startsWith($value, "coolify."));
            $this->application->custom_labels = base64_encode(
                $labels->implode("\n")
            );
            $this->application->save();
        } elseif (
            $this->application->settings->is_container_label_readonly_enabled ||
            $this->pull_request_id !== 0
        ) {
            $labels = collect(
                generateLabelsApplication($this->application, $this->preview)
            );
        }
        ...
        if (
            ...
            $this->execute_remote_command([
                executeInDocker(
                    $this->deployment_uuid,
                    "cat {$this->workdir}{$this->dockerfile_location}"
                ),
                "hidden" => true,
                "save" => "dockerfile_from_repo",
                "ignore_errors" => true,
            ]);
            $this->application->parseHealthcheckFromDockerfile(
                $this->saved_outputs->get("dockerfile_from_repo")
            );
        }

        $custom_network_aliases = is_array(
            $this->application->custom_network_aliases
        )
            ? $this->application->custom_network_aliases
            : [];
        ...
        $docker_compose = [
            "services" => [
                $this->container_name => [
                    ...
                    "mem_swappiness" =>
                        $this->application->limits_memory_swappiness,
                    "mem_reservation" =>
                        $this->application->limits_memory_reservation,
        ...
        $docker_compose["services"][$this->container_name]["healthcheck"] = [
            ...
            "start_period" =>
                $this->application->health_check_start_period . "s",
        ];

        if (!is_null($this->application->limits_cpuset)) {
            data_set(
                $docker_compose,
                "services." . $this->container_name . ".cpuset",
                $this->application->limits_cpuset
            );
        }
        ...
        if ($this->server->isSwarm()) {
            ...
            if (data_get($this->application, "swarm_placement_constraints")) {
                $swarm_placement_constraints = Yaml::parse(
                    base64_decode(
                        data_get(
                            $this->application,
        ...
        if (
            $this->server->isLogDrainEnabled() &&
            $this->application->isLogDrainEnabled()
        ) {
            $docker_compose["services"][$this->container_name][
                "logging"
        ...
        if ($this->application->settings->is_gpu_enabled) {
            $gpu = [
                ...
                "options" => data_get(
                    $this->application,
                    "settings.gpu_options",
                    []
                ),
            ];
            if (data_get($this->application, "settings.gpu_count")) {
                $gpu["count"] =
                    data_get($this->application, "settings.gpu_count") === "all"
                        ? "all"
                        : (int) data_get(
                            $this->application,
                            "settings.gpu_count"
            ...
            data_set(
                $docker_compose,
                "services.{$this->container_name}.deploy.resources.reservations.devices",
                [$gpu]
            );
        }
        ...
        if (
            ...
            $docker_compose["services"][$this->container_name]["ports"] =
                $this->application->ports_mappings_array;
        }
        ...
        if (count($persistent_file_volumes) > 0) {
            $docker_compose["services"][$this->container_name][
                ...
                $persistent_file_volumes
                    ->map(fn($item) => "$item->fs_path:$item->mount_path")
                    ->toArray()
            );
        }
        ...
        if ($this->pull_request_id === 0) {
            $custom_compose = convertDockerRunToCompose(
                $this->application->custom_docker_run_options
            );
            ...
            if (
                ...
                $docker_compose["services"][$this->application->uuid] =
                    $docker_compose["services"][$this->container_name];
        ...
        $this->execute_remote_command([
            executeInDocker(
                $this->deployment_uuid,
                "echo '{$this->docker_compose_base64}' | base64 -d | tee {$this->workdir}/docker-compose.yaml > /dev/null"
            ),
            "hidden" => true,
        ]);
    }
    ...
    private function generate_local_persistent_volumes()
    {
        ...
        foreach ($this->application->persistentStorages as $persistentStorage) {
            if (
                $persistentStorage->host_path !== "" &&
                $persistentStorage->host_path !== null
            ) {
            ...
            $local_persistent_volumes[] =
                $volume_name . ":" . $persistentStorage->mount_path;
        }
    ...
    private function generate_healthcheck_commands()
    {
        ...
        if (
            $this->application->settings->is_static ||
            $this->application->build_pack === "static"
        ) {
    ...
    {
        $this->application_deployment_queue->addLogEntry(
            "Pulling latest image ($image) from the registry."
        );
        $this->execute_remote_command([
    ...
    private function build_image()
    {
        ...
        if (
            ...
            if ($this->application->static_image) {
                ...
                $this->application_deployment_queue->addLogEntry(
                    "Continuing with the building process."
                );
            }
            if ($this->application->build_pack === "static") {
...
COPY ./nginx.conf /etc/nginx/conf.d/default.conf");
                if (
                    str(
                        $this->application->custom_nginx_configuration
                    )->isNotEmpty()
                ) {
                    $nginx_config = base64_encode(
                        $this->application->custom_nginx_configuration
                    );
                } else {
                    if ($this->application->settings->is_spa) {
                        $nginx_config = base64_encode(
                            defaultNginxConfiguration("spa")
                        );
                    } else {
                        $nginx_config = base64_encode(
                            defaultNginxConfiguration()
                        );
                    }
                }
            } else {
                if ($this->application->build_pack === "nixpacks") {
                    ...
                    $this->execute_remote_command([
                        executeInDocker(
                            $this->deployment_uuid,
                            "echo '{$this->nixpacks_plan}' | base64 -d | tee /artifacts/thegameplan.json > /dev/null"
                        ),
                        "hidden" => true,
                    ]);
                    if ($this->force_rebuild) {
                        $this->execute_remote_command([
                            executeInDocker(
                                $this->deployment_uuid,
                                "nixpacks build -c /artifacts/thegameplan.json --no-cache --no-error-without-start -n {$this->build_image_name} {$this->workdir} -o {$this->workdir}"
                            ),
                            "hidden" => true,
                        ]);
                        ...
                        $this->execute_remote_command([
                            executeInDocker(
                                $this->deployment_uuid,
                                "nixpacks build -c /artifacts/thegameplan.json --cache-key '{$this->application->uuid}' --no-error-without-start -n {$this->build_image_name} {$this->workdir} -o {$this->workdir}"
                            ),
                            "hidden" => true,
                        ]);
                    ...
                    $this->execute_remote_command(
                        [
                            executeInDocker(
                                $this->deployment_uuid,
                                "echo '{$base64_build_command}' | base64 -d | tee /artifacts/build.sh > /dev/null"
                            ),
                            "hidden" => true,
                        ],
                        [
...
COPY ./nginx.conf /etc/nginx/conf.d/default.conf");
            ...
            $this->execute_remote_command(
                [
                    executeInDocker(
                        $this->deployment_uuid,
                        "echo '{$dockerfile}' | base64 -d | tee {$this->workdir}/Dockerfile > /dev/null"
                    ),
                ],
                [
                    executeInDocker(
                        $this->deployment_uuid,
                        "echo '{$nginx_config}' | base64 -d | tee {$this->workdir}/nginx.conf > /dev/null"
                    ),
                ],
                [
                    executeInDocker(
                ...
                if ($this->application->build_pack === "nixpacks") {
                    ...
                    if ($this->force_rebuild) {
                        $this->execute_remote_command([
                            executeInDocker(
                                $this->deployment_uuid,
                                "nixpacks build -c /artifacts/thegameplan.json --no-cache --no-error-without-start -n {$this->production_image_name} {$this->workdir} -o {$this->workdir}"
                            ),
                            "hidden" => true,
                        ]);
                        ...
                        $this->execute_remote_command([
                            executeInDocker(
                                $this->deployment_uuid,
                                "nixpacks build -c /artifacts/thegameplan.json --cache-key '{$this->application->uuid}' --no-error-without-start -n {$this->production_image_name} {$this->workdir} -o {$this->workdir}"
                            ),
                            "hidden" => true,
                        ]);
            ...
            if (
                ...
                if ($this->pull_request_id === 0) {
                    $containers = $containers->filter(function ($container) {
                        return data_get($container, "Names") !==
                            $this->container_name &&
                ...
                $containers->each(function ($container) {
                    $this->graceful_shutdown_container(
                        data_get($container, "Names")
                    );
                });
            }
    ...
    private function start_by_compose_file()
    {
        if ($this->application->build_pack === "dockerimage") {
            ...
            if ($this->use_build_server) {
                $this->execute_remote_command([
                    "{$this->coolify_variables} docker compose --project-name {$this->application->uuid} --project-directory {$this->configuration_dir} -f {$this->configuration_dir}{$this->docker_compose_location} up --pull always --build -d",
                    "hidden" => true,
                ]);
            } else {
                $this->execute_remote_command([
    ...
    private function add_build_env_variables_to_dockerfile()
    {
        ...
        $dockerfile = collect(
            str($this->saved_outputs->get("dockerfile"))
                ->trim()
                ->explode("\n")
        );
        if ($this->pull_request_id === 0) {
    ...
    private function run_pre_deployment_command()
    {
        ...
        foreach ($containers as $container) {
            ...
            if (
                ...
                str_starts_with(
                    $containerName,
                    $this->application->pre_deployment_command_container .
                        "-" .
                        $this->application->uuid
                )
            ) {
    ...
    private function run_post_deployment_command()
    {
        ...
        foreach ($containers as $container) {
            ...
            if (
                ...
                str_starts_with(
                    $containerName,
                    $this->application->post_deployment_command_container .
                        "-" .
                        $this->application->uuid
                )
            ) {
                ...
                try {
                    ...
                    $post_deployment_command_output = $this->saved_outputs->get(
                        "post-deployment-command-output"
                    );
                    if ($post_deployment_command_output) {
                        ...
                        $this->application_deployment_queue->addLogEntry(
                            $post_deployment_command_output,
                            "stderr"
                        );
                    }
                }
    ...
    private function next(string $status)
    {
        ...
        if (
            $this->application_deployment_queue->status ===
        ...
        if ($this->application->build_pack !== "dockercompose") {
            ...
            if ($code !== 69420) {
                ...
                if (
                    ...
                    $this->execute_remote_command([
                        "docker rm -f $this->container_name >/dev/null 2>&1",
                        "hidden" => true,
                        "ignore_errors" => true,
                    ]);
                }
