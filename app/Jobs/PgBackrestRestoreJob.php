...
class PgBackrestRestoreJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            try {
                $this->restore->appendLog('Capturing PGDATA ownership.');
                $this->getPgDataOwnership(); // Capture ownership before wiping
                ...
                $this->restore->appendLog('Fixing PGDATA ownership.');
                $this->fixPgDataOwnership(); // Restore ownership after sidecar writes
    ...
    private string $originalOwner = '999:999'; // Default to standard Postgres

    private function getPgDataOwnership(): void
    {
        $server = $this->database->destination->server;
        $pgdataVolume = $this->database->pgdataVolume();
        $pgdataMount = $pgdataVolume->host_path ?: $pgdataVolume->name;

        // Use stat to get uid:gid of the root of pgdata
        $cmd = sprintf(
            'docker run --rm -v %s:/var/lib/postgresql/data alpine stat -c "%%u:%%g" /var/lib/postgresql/data',
            escapeshellarg($pgdataMount)
        );

        $output = instant_remote_process([$cmd], $server, false, false, 60, disableMultiplexing: true);

        if ($output && preg_match('/^\d+:\d+$/', trim($output))) {
            $this->originalOwner = trim($output);
        }
    }

    private function fixPgDataOwnership(): void
    {
        $server = $this->database->destination->server;
        $pgdataVolume = $this->database->pgdataVolume();
        $pgdataMount = $pgdataVolume->host_path ?: $pgdataVolume->name;
        ...
        $cmd = sprintf(
            'docker run --rm -v %s:/var/lib/postgresql/data alpine chown -R %s /var/lib/postgresql/data',
            escapeshellarg($pgdataMount),
            escapeshellarg($this->originalOwner)
        );

        instant_remote_process([$cmd], $server, true, false, 300, disableMultiplexing: true);
    }
    ...
    private function runRestoreSidecar(string $stanza): void
    {
        ...
        $cmd = PgBackrestService::buildSidecarRestoreCommand(
            $stanza,
            $backup,
            $network,
            $mounts,
