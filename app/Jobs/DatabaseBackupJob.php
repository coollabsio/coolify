...
class DatabaseBackupJob implements ShouldBeEncrypted, ShouldQueue
{
    ...
    public function handle(): void
    {
        try {
            ...
            foreach ($databasesToBackup as $database) {
                ...
                try {
                    if (str($databaseType)->contains('postgres')) {
                        // Check if pgBackRest engine is selected
    ...
    private function backupWithPgbackrest(string $database): void
    {
        try {
            ...
            // Ensure pgBackRest is set up in the container
            ...
            // Save backup info to a file on the server
            $commands = [
            ...
            // pgBackRest handles S3 natively, so mark as uploaded if S3 is configured
            if ($this->s3) {
