...
class InstallDocker
{
    ...
    public function handle(Server $server)
    {
        ...
        if (isDev() && $server->id === 0) {
            ...
            // Use OpenRC for Alpine, systemd for everything else
    ...
    {
        // Dynamic codename fallback: Check if Docker repo has the distribution, fallback to bookworm if not.
        // This handles Debian 13 (trixie) and future releases where Docker may not have packages yet.
        return "curl --max-time 300 --retry 3 https://releases.rancher.com/install-docker/{$this->dockerVersion}.sh | sh || curl --max-time 300 --retry 3 https://get.docker.com | sh -s -- --version {$this->dockerVersion} || (".
            '. /etc/os-release && '.
            'install -m 0755 -d /etc/apt/keyrings && '.
            'curl -fsSL https://download.docker.com/linux/${ID}/gpg -o /etc/apt/keyrings/docker.asc && '.
            'chmod a+r /etc/apt/keyrings/docker.asc && '.
            // Dynamic fallback: check if the codename exists in Docker repo, otherwise use bookworm
            'DOCKER_CODENAME=${VERSION_CODENAME} && '.
            'if ! curl -fsSL --head "https://download.docker.com/linux/${ID}/dists/${VERSION_CODENAME}/Release" >/dev/null 2>&1; then '.
            'DOCKER_CODENAME="bookworm"; '.
            'echo "Docker repo not available for ${VERSION_CODENAME}, falling back to ${DOCKER_CODENAME}"; '.
            'fi && '.
    ...
    {
        // Alpine Linux uses apk package manager and OpenRC instead of systemd
        // docker-cli-compose provides docker compose v2 functionality
        return 'apk update && '.
