...
class StartInfluxdb
{
    ...
    public function handle(StandaloneInfluxdb $database, ?Activity $activity = null)
    {
            ...
            'services' => [
                $container_name => [
                    ...
                    'labels' => defaultDatabaseLabels($this->database)
                        ->merge(generateLabelsDatabase($this->database, $this->database->httpPort()))
                        ->toArray(),
                    'healthcheck' => $this->database->healthCheckConfiguration([
                        'CMD', 'influx', 'ping', '--host', 'http://127.0.0.1:8086',
                    ]),
    ...
    private function generate_environment_variables()
    {
        ...
        $defaults = [
            ...
            'DOCKER_INFLUXDB_INIT_USERNAME' => $this->database->influxdb_admin_user,
            'DOCKER_INFLUXDB_INIT_PASSWORD' => $this->database->influxdb_admin_password,
            'DOCKER_INFLUXDB_INIT_ORG' => $this->database->influxdb_org,
            'DOCKER_INFLUXDB_INIT_BUCKET' => $this->database->influxdb_bucket,
            'DOCKER_INFLUXDB_INIT_ADMIN_TOKEN' => $this->database->influxdb_admin_token,
        ];

        foreach ($defaults as $key => $value) {
            if ($environment_variables->filter(fn ($env) => str($env)->startsWith("$key="))->isEmpty()) {
