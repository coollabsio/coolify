...
class StartCassandra
{
    ...
    private function generate_environment_variables()
    {
        ...
        if ($environment_variables->filter(fn ($env) => str($env)->contains('CASSANDRA_USER'))->isEmpty()) {
            $environment_variables->push("CASSANDRA_USER={$this->database->cassandra_admin_user}");
        }

        if ($environment_variables->filter(fn ($env) => str($env)->contains('CASSANDRA_PASSWORD'))->isEmpty()) {
            $environment_variables->push("CASSANDRA_PASSWORD={$this->database->cassandra_admin_password}");
        }

        if ($environment_variables->filter(fn ($env) => str($env)->contains('CASSANDRA_CLUSTER_NAME'))->isEmpty()) {
            $environment_variables->push('CASSANDRA_CLUSTER_NAME=Coolify Cluster');
        }
