...
class StartSurrealDB
{
        ...
        $docker_compose = [
            ...
            'networks' => [
                $this->database->destination->network => [
                    'external' => true, 'name' => $this->database->destination->network, 'attachable' => true,
                ],
            ],
        ];
    ...
    private function generate_local_persistent_volumes_only_volume_names()
    {
        $names = [];
        foreach ($this->database->persistentStorages as $storage) {
            if ($storage->host_path) continue;
            $names[$storage->name] = ['name' => $storage->name, 'external' => false];
        }
        return $names;
    }
