...
class BackupEdit extends Component
{
    ...
    public ?string $newAgeKeyName = null;

    #[Validate(['nullable', 'string'])]
    public ?string $newAgeKeyPublicKey = null;

    #[Locked]
    public ?string $generatedAgeKeyPrivateKey = null;

    #[Locked]
    public ?string $generatedAgeKeyPublicKey = null;
    ...
    public function generateAgeKey(): void
    {
        try {
            ...
            // The private key is shown to the user exactly once and is never sent to the
            // server again or persisted — only the public key gets saved, below.
            $this->generatedAgeKeyPrivateKey = $keyPair['private_key'];
            $this->generatedAgeKeyPublicKey = $keyPair['public_key'];
            $this->newAgeKeyName = $keyPair['name'];
        } catch (\Throwable $e) {
    ...
    public function confirmGeneratedAgeKeySaved(): void
    {
        try {
            ...
            if (blank($this->generatedAgeKeyPublicKey)) {
                return;
            }

            $key = AgeKey::create([
                'name' => filled($this->newAgeKeyName) ? $this->newAgeKeyName : generate_random_name(),
                'description' => 'Generated by Coolify',
                'public_key' => $this->generatedAgeKeyPublicKey,
                'team_id' => currentTeam()->id,
            ]);

            $this->generatedAgeKeyPrivateKey = null;
            $this->generatedAgeKeyPublicKey = null;
            $this->newAgeKeyName = null;
            $this->refreshAvailableAgeKeys();
            $this->ageKeyId = $key->id;
            $this->dispatch('success', "Age key \"{$key->name}\" saved.");
        } catch (\Throwable $e) {
    ...
    public function addExistingAgeKey(): void
    {
        try {
            $this->authorize('manageBackups', $this->backup->database);

            $this->validate([
                'newAgeKeyName' => ['required', 'string', 'max:255'],
                'newAgeKeyPublicKey' => ['required', 'string'],
            ]);

            $key = AgeKey::create([
                'name' => $this->newAgeKeyName,
                'description' => 'Added by user',
                'public_key' => $this->newAgeKeyPublicKey,
                'team_id' => currentTeam()->id,
            ]);

            $this->newAgeKeyName = null;
            $this->newAgeKeyPublicKey = null;
            $this->refreshAvailableAgeKeys();
            $this->ageKeyId = $key->id;
            $this->dispatch('success', "Age key \"{$key->name}\" added.");
        } catch (\Illuminate\Validation\ValidationException $e) {
            $this->dispatch('error', collect($e->errors())->flatten()->first() ?? 'Invalid age key.');
        } catch (\Throwable $e) {
            $this->dispatch('error', 'Failed to add age key: '.$e->getMessage());
        }
    }

    private function refreshAvailableAgeKeys(): void
    {
        $this->availableAgeKeys = AgeKey::ownedByCurrentTeam()->get();
    }
