...
class Networking extends Component
{
    ...
    public function availableNetworks(): Collection
    {
        ...
        return DockerNetwork::query()
            ...
            ->whereNotIn('id', $this->attachmentsQuery()->pluck('docker_network_id'))
    ...
    public function networkTypeLabel(?DockerNetwork $network): string
    {
        ...
        return $network->managed_by_coolify ? 'Created by Coolify' : 'External to Coolify';
    }

    public function attachmentModeLabel(NetworkAttachment $attachment): string
    {
        if ($attachment->is_runtime_discovered) {
            return 'Runtime only';
        }

        if ($attachment->is_managed) {
            return 'Configured';
        }
    ...
    public function attachmentModeType(NetworkAttachment $attachment): string
    {
        if ($attachment->is_runtime_discovered) {
            return 'warning';
        }
    ...
    private function refreshKnownNetworks(bool $force = false): void
    {
        ...
        app(ResourceNetworkReconciler::class)->reconcile($this->resource);
    ...
    private function attachmentsQuery()
    {
