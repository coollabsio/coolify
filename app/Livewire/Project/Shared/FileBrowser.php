...
class FileBrowser extends Component
{
    ...
    public function loadContainers()
    {
        foreach ($this->servers as $server) {
            if (data_get($this->parameters, 'application_uuid')) {
                if ($server->isSwarm()) {
                    continue; // Swarm not supported for file browsing
                }
    ...
    public function listDirectory(?string $path = null)
    {
        if ($this->selectedContainer === 'default') {
        ...
        try {
            ...
            // Format: type|permissions|owner|group|size|mtime|name
    ...
    private function parseLsOutput(string $output): array
    {
        ...
        foreach ($lines as $line) {
            ...
            // Parse ls -la output
            // drwxr-xr-x   2 root root  4096 Jan 15 12:00 dirname
            // -rw-r--r--   1 root root  1234 Jan 15 12:00 filename
            // lrwxrwxrwx   1 root root    11 Jan 15 12:00 link -> target
    ...
    public function navigateTo(string $name, string $type)
    {
    ...
    public function downloadFile(string $name)
    {
        ...
        try {
            ...
            if ($fileSize === 0) {
                // Could be an empty file or stat failed - try to get it anyway
            }
    ...
    public function deleteEntry(string $name, string $type)
    {
        if ($this->selectedContainer === 'default') {
            $this->dispatch('error', 'Please select a container.');
            return;
