...
class Application extends BaseModel
{
    ...
    public function generateGitImportCommands(string $deployment_uuid, int $pull_request_id = 0, ?string $git_type = null, bool $exec_in_docker = true, bool $only_checkout = false, ?string $custom_base_dir = null, ?string $commit = null)
    {
        ...
        if ($this->deploymentType() === 'other') {
            if (str_contains($customRepository, '@')) {
                // SSH URL
                $fullRepoUrl = $customRepository;
            } elseif (str_contains($customRepository, '://')) {
                // HTTPS URL
                $fullRepoUrl = $customRepository;
            } else {
                // Construct URL based on git host
                $gitHost = $this->git_host ?? 'github.com';
                if (str_contains($gitHost, 'gitlab')) {
                    $fullRepoUrl = "https://{$gitHost}/{$customRepository}.git";
                } elseif (str_contains($gitHost, 'bitbucket')) {
                    $fullRepoUrl = "https://{$gitHost}/{$customRepository}.git";
                } else {
                    $fullRepoUrl = "https://{$gitHost}/{$customRepository}";
                }
            }
