...
class StandaloneValkey extends BaseModel
{
    ...
    protected static function booted()
    {
        static::created(function ($database) {
            LocalPersistentVolume::create([
                'name' => 'valkey-data-'.$database->uuid,
                'mount_path' => '/data',
                'host_path' => null,
    ...
    {
        return StandaloneValkey::whereRelation('environment.project.team', 'id', currentTeam()->id)->orderBy('name');
    }
    ...
    {
        $newConfigHash = $this->image.$this->ports_mappings.$this->valkey_conf;
    ...
    protected function internalDbUrl(): Attribute
    {
        return new Attribute(
            get: function () {
                $valkey_version = $this->getValkeyVersion();
                $username_part = version_compare($valkey_version, '6.0', '>=') ? rawurlencode($this->valkey_username).':' : '';
                $encodedPass = rawurlencode($this->valkey_password);
                $scheme = $this->enable_ssl ? 'valkey+ssl' : 'valkey';
    ...
    public function getValkeyVersion()
    {
    ...
    public function valkeyPassword(): Attribute
    {
        return new Attribute(
            get: function () {
                $password = $this->runtime_environment_variables()->where('key', 'VALKEY_PASSWORD')->first();
                if (! $password) {
                    return null;
                }
