...
class DockerNetworkCatalogRefresher
{
    ...
    public function __construct(
        ...
        private readonly DestinationNetworkSynchronizer $destinationSynchronizer = new DestinationNetworkSynchronizer,
    ) {}
    ...
    public function refresh(Server $server, bool $force = false): Collection
    {
        ...
        if (! $force && is_array($cached) && $this->isFresh($cached)) {
            return collect([
                ...
                'removed' => data_get($cached, 'removed', 0),
        ...
        $this->destinationSynchronizer->sync($server);
