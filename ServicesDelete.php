...
class ServicesDelete extends Command
{
    ...
    private function promptAndValidatePassword(): bool
    {
        $userPassword = password('Please enter your password to confirm this destructive action:');
    ...
    // Dummy password check, replace with real logic (e.g., check against env or DB)
    private function checkPassword($password): bool
    {
        // Example: check against an environment variable
        return $password === env('ADMIN_PASSWORD', 'your_default_password');
    }
    ...
    private function deleteServer()
    {
        $servers = Server::all();
        if ($servers->count() === 0) {
            $this->error('There are no servers to delete.');
            return;
        }
        ...
        foreach ($serversToDelete as $server) {
            ...
            if ($toDelete) {
                $this->info($toDelete);
                $confirmed = confirm('Are you sure you want to delete this server?');
                if (! $confirmed) {
                    continue;
                }
                if (!$this->promptAndValidatePassword()) {
                    continue;
                }
                $toDelete->delete();
    ...
    private function deleteApplication()
    {
        ...
        foreach ($applicationsToDelete as $application) {
            ...
            if ($toDelete) {
                $this->info($toDelete);
                $confirmed = confirm('Are you sure you want to delete this application?');
                if (! $confirmed) {
                    continue;
                }
                if (!$this->promptAndValidatePassword()) {
                ...
                $this->info('Application deletion dispatched.');
            }
        }
    }
    ...
    private function deleteDatabase()
    {
        ...
        foreach ($databasesToDelete as $databaseKey) {
            ...
            if ($toDelete) {
                $this->info($toDelete);
                $confirmed = confirm('Are you sure you want to delete this database?');
                if (! $confirmed) {
                    continue;
                }
                if (!$this->promptAndValidatePassword()) {
                ...
                DeleteResourceJob::dispatch($toDelete);
                $this->info('Database deletion dispatched.');
            }
        }
    }
    ...
    private function deleteService()
    {
        ...
        foreach ($servicesToDelete as $service) {
            ...
            if ($toDelete) {
                $this->info($toDelete);
                $confirmed = confirm('Are you sure you want to delete this service?');
                if (! $confirmed) {
                    continue;
                }
                if (!$this->promptAndValidatePassword()) {
                ...
                DeleteResourceJob::dispatch($toDelete);
                $this->info('Service deletion dispatched.');
            }
        }
    }
}
