	...
	function doSearch(bang?: string) {
		...
		if (search) {
			if (search.startsWith('!')) {
				...
				filtered.gitSources = gitSources.filter((source: any) => {
					return (
				...
				filtered.destinations = destinations.filter((destination: any) => {
					return (
	...
	{#if applications.length !== 0 || destinations.length !== 0 || databases.length !== 0 || services.length !== 0 || gitSources.length !== 0 || destinations.length !== 0}
		<div class="form-control">
			<div class="input-group flex w-full">
				<div class="btn btn-square cursor-default no-animation">
					<svg
