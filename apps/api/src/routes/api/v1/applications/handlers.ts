...
export async function getApplication(request: FastifyRequest<OnlyId>) {
	try {
		const { id } = request.params;
		const { teamId } = request.user;
		const appId = process.env['OUTPOST_APP_ID'];
		const application: any = await getApplicationFromDB(id, teamId);
...
export async function restartApplication(request: FastifyRequest<OnlyId>, reply: FastifyReply) {
	try {
		...
		if (application?.destinationDockerId) {
			...
			const labels = [];
			let image = null;
			const { stdout: container } = await executeDockerCmd({
				dockerId,
...
export async function loadPreviews(request: FastifyRequest<OnlyId>) {
	try {
		...
		const containers = formatLabelsOnDocker(stdout).filter(
			(container) =>
				container.Labels['outpost.configuration'] &&
				container.Labels['outpost.type'] === 'standalone-application'
		);

		const jsonContainers = containers
			.map((container) =>
				JSON.parse(Buffer.from(container.Labels['outpost.configuration'], 'base64').toString())
			)
			.filter((container) => {
	...
	try {
		...
		if (destinationDockerId) {
			try {
				...
				const { stdout, stderr } = await executeDockerCmd({
					dockerId,
...
export async function getBuilds(request: FastifyRequest<GetBuilds>) {
	try {
		...
		let { skip = 0, limit = 200 } = request.query;
		if (typeof skip !== 'number') {
			skip = Number(skip);
		}

		if (typeof limit !== 'number') limit = Number(limit) || 200;

		[];

		const buildCount = await prisma.build.count({ where: { applicationId: id } });
		let builds = await (buildId
			? prisma.build.findMany({
					where: { applicationId: id, id: buildId },
					take: limit
			  })
			: prisma.build.findMany({
					where: { applicationId: id },
