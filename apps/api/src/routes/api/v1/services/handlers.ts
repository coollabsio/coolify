		...
		if (otherFqdns && otherFqdns.length > 0) {
			for (const ofqdn of otherFqdns) {
				...
				if (found) {
					throw {
						status: 500,
						message: `Domain ${getDomain(ofqdn).replace('www.', '')} is already in use!`
					};
				}
			}
		}
...
async function startWordpressService(request: FastifyRequest<ServiceStartStop>) {
	try {
		...
		const config = {
			...
			mysql: {
				image: `mysql:5.7`,
				volume: `${id}-mysql-data:/var/lib/mysql`,
				environmentVariables: {
...
async function startGhostService(request: FastifyRequest<ServiceStartStop>) {
	try {
		...
		const config = {
			ghost: {
				image: `${image}:${version}`,
				volume: `${id}-ghost:/var/lib/ghost/content`,
				environmentVariables: {
					url: fqdn,
					GHOST_HOST: domain,
			...
			mariadb: {
				...
				volume: `${id}-mariadb:/var/lib/mysql`,
				environmentVariables: {
...
async function startMoodleService(request: FastifyRequest<ServiceStartStop>) {
	try {
		...
		const config = {
			...
			mariadb: {
				image: 'mariadb:latest',
				volume: `${id}-mariadb-data:/var/lib/mysql`,
				environmentVariables: {
					MARIADB_USER: mariadbUser,
					MARIADB_PASSWORD: mariadbPassword,
					MARIADB_DATABASE: mariadbDatabase,
