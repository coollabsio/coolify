...
Release workflows never edit or commit versions. Stable versions come from `config/constants.php`; the intended RC base comes from `coolify.nightly.version` in the root `versions.json`.
...
2. Set `coolify.nightly.version` in the root `versions.json` to the intended RC, such as `4.4-rc.1`.
3. Regular `next` builds publish `sha-<short-sha>`, `4.4-rc.1.<short-sha>`, and the moving `next` tag. The nightly installer follows the moving `next` tag. They never publish the exact `4.4-rc.1` tag.
...
7. Advance `coolify.nightly.version` in the root `versions.json` to the next intended RC version.
