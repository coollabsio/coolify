<div x-data="{
    ...
    <div class="relative">
        <button @click="dropdownOpen = !dropdownOpen"
            ...
            title="{{ __('common.preferences') }}">
            <!-- Sliders Icon -->
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" title="{{ __('common.preferences') }}">
            ...
            <div
                ...
                <div class="py-4 flex-1 overflow-y-auto scrollbar">
                    ...
                    <div x-show="filteredEntries.length === 0" class="text-center py-8">
                        ...
                        <h3 class="mt-2 text-sm font-medium dark:text-white">{{ __('upgrade.no_updates_found') }}</h3>
                        <p class="mt-1 text-sm dark:text-neutral-400">
                            <span x-show="search.trim() !== ''">{{ __('upgrade.no_updates_match_search') }}</span>
                            <span x-show="search.trim() === ''">{{ __('upgrade.no_updates_available') }}</span>
                        </p>
                    </div>
                </div>
            </div>
