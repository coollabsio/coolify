<div>
    ...
    <div class="flex flex-wrap gap-2 ">
        @forelse ($tags as $oneTag)
            <a wire:navigate.hover :class="{{ $tag?->id == $oneTag->id }} && 'dark:bg-coollabs'"
        ...
        <div>
            ...
            <div class="flex items-end gap-2 ">
                ...
                <x-modal-confirmation title="Redeploy all resources with this tag?" isHighlighted buttonTitle="Redeploy All"
                    submitAction="redeployAll" :actions="[
            ...
            <div class="grid grid-cols-1 gap-2 pt-4 lg:grid-cols-2 xl:grid-cols-3">
                @if (isset($applications) && count($applications) > 0)
                    @foreach ($applications as $application)
                        <a wire:navigate.hover href="{{ $application->link() }}" class="box group">
                ...
                @if (isset($services) && count($services) > 0)
                    @foreach ($services as $service)
                        <a wire:navigate.hover href="{{ $service->link() }}" class="flex flex-col box group">
                ...
                @forelse ($deploymentsPerTagPerServer as $serverName => $deployments)
                    ...
                    <div class="grid grid-cols-1 gap-2 lg:grid-cols-3">
                        @foreach ($deployments as $deployment)
                            <a wire:navigate.hover href="{{ data_get($deployment, 'deployment_url') }}" @class([
