    ...
    <form wire:submit="submit" class="flex flex-col gap-2">
        ...
        <div class="flex gap-2">
            ...
            <x-forms.input label="Image" id="image" required canGate="update" :canResource="$database"
                helper="For all available images, check here:<br><br><a target='_blank' href='https://hub.docker.com/r/valkey/valkey'>https://hub.docker.com/r/valkey/valkey</a>" />
        </div>
        <div class="flex flex-col gap-2">
            @if ($database->started_at)
                ...
                <div class="flex gap-2">
                    <x-forms.input label="Password" id="valkeyPassword" type="password"
                        helper="You can only change this in the database." canGate="update" :canResource="$database" />
                </div>
            @else
                <div class="pt-2 dark:text-warning">You can only change the password in the database after
                    initial start.</div>
                <div class="flex gap-2">
                    <x-forms.input label="Password" id="valkeyPassword" type="password" required
                        helper="You can change the Valkey Password in the input field below or by editing the value of the VALKEY_PASSWORD environment variable.
                <br><br>
                If you change the Valkey Password in the database, please sync it here, otherwise automations (like backups) won't work.
                <br><br>
                Note: If the environment variable VALKEY_PASSWORD is set as a shared variable (environment, project, or team-based), this input field will become read-only."
                        :disabled="$this->isSharedVariable('VALKEY_PASSWORD')" canGate="update" :canResource="$database" />
                </div>
            @endif
        </div>
        <x-forms.input
...
# timeout 300"
            helper="You only need to provide the Valkey directives you want to override — Valkey will use default values for everything else. <br/><br/>
...
🔗 <strong>Tip:</strong> <a target='_blank' class='underline dark:text-white' href='https://github.com/valkey-io/valkey/tree/unstable/valkey.conf'>View the Valkey default configuration</a> to see what options are available."
            label="Custom Valkey Configuration" rows="10" id="valkeyConf" canGate="update"
            :canResource="$database" />
