    ...
    <div class="navbar-main">
        <nav
            ...
            <a wire:navigate.hover
                class="{{ request()->routeIs('project.database.configuration') ? 'dark:text-white' : '' }}"
                href="{{ route('project.database.configuration', $parameters) }}">
                Configuration
            </a>

            <a wire:navigate.hover class="{{ request()->routeIs('project.database.logs') ? 'dark:text-white' : '' }}"
                href="{{ route('project.database.logs', $parameters) }}">
                Logs
            </a>
            @can('canAccessTerminal')
                <a wire:navigate.hover class="{{ request()->routeIs('project.database.command') ? 'dark:text-white' : '' }}"
            ...
            <div class="flex flex-wrap gap-2 items-center">
                @if (!str($database->status)->startsWith('exited'))
                    <x-modal-confirmation title="Confirm Database Restart?" buttonTitle="Restart" submitAction="restart"
                        ...
                        step2ButtonText="Restart Database" :dispatchEvent="true" dispatchEventType="restartEvent">
                        <x-slot:button-title>
