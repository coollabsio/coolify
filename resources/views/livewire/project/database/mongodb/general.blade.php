    ...
    <form wire:submit="submit" class="flex flex-col gap-2">
        ...
        <div class="flex gap-2">
            ...
            <x-forms.input label="{{ __('application.image') }}" id="image" required canGate="update" :canResource="$database"
                helper="{{ __('database.image_helper_mongodb') }}" />
        </div>
        <div class="pt-2 dark:text-warning">{{ __('database.sync_warning') }}
        </div>
        @if ($database->started_at)
            <div class="flex xl:flex-row flex-col gap-2">
                <x-forms.input label="{{ __('database.initial_username') }}" id="mongoInitdbRootUsername"
                    placeholder="{{ __('database.username_placeholder') }}"
                    helper="{{ __('database.username_helper') }}"
                    canGate="update" :canResource="$database" />
                <x-forms.input label="{{ __('database.initial_password') }}" id="mongoInitdbRootPassword" type="password"
                    required
                    helper="{{ __('database.password_helper') }}"
                    canGate="update" :canResource="$database" />
                <x-forms.input label="{{ __('database.initial_database') }}" id="mongoInitdbDatabase"
        ...
        @else
            <div class="flex xl:flex-row flex-col gap-2 pb-2">
                <x-forms.input required label="{{ __('database.username') }}" id="mongoInitdbRootUsername"
                    placeholder="{{ __('database.username_placeholder') }}" canGate="update" :canResource="$database" />
                <x-forms.input label="{{ __('database.password') }}" id="mongoInitdbRootPassword" type="password" required
                    canGate="update" :canResource="$database" />
                <x-forms.input required label="{{ __('database.database') }}" id="mongoInitdbDatabase"
        ...
        <div class="flex flex-col gap-2">
                    ...
                    <div class="mx-2">
                        @if (str($database->status)->contains('exited'))
                            <x-forms.select id="sslMode" label="{{ __('database.ssl_mode') }}" wire:model.live="sslMode"
                                ...
                                helper="{{ __('database.ssl_mode_helper_mongodb') }}" canGate="update"
                                :canResource="$database">
                        ...
                        @else
                            <x-forms.select id="sslMode" label="{{ __('database.ssl_mode') }}" instantSave="instantSaveSSL"
                                disabled helper="{{ __('database.db_stopped_to_change') }}" canGate="update"
                                :canResource="$database">
                                <option value="allow" title="{{ __('database.ssl_mode_allow_title') }}">{{ __('database.ssl_mode_allow') }}</option>
                                <option value="prefer" title="{{ __('database.ssl_mode_prefer_title') }}">{{ __('database.ssl_mode_prefer') }}</option>
                                <option value="require" title="{{ __('database.ssl_mode_require_title') }}">{{ __('database.ssl_mode_require') }}</option>
            ...
            <x-forms.textarea label="{{ __('database.custom_mongodb_config') }}" rows="10" id="mongoConf"
                canGate="update" :canResource="$database" />
