    ...
    <form wire:submit="submit" class="flex flex-col gap-2">
        ...
        <div class="flex gap-2">
            ...
            <x-forms.input label="Image" id="image" required canGate="update" :canResource="$database"
                helper="For all available images, check here:<br><br><a target='_blank' href='https://hub.docker.com/r/bitnami/cassandra/'>https://hub.docker.com/r/bitnami/cassandra/</a>" />
        </div>

        @if ($database->started_at)
            <div class="flex gap-2">
                <x-forms.input label="Initial Username" id="cassandraAdminUser" placeholder="If empty: cassandra"
                ...
                @else
                    <x-forms.input label="Initial Password" id="cassandraAdminPassword" type="password" required readonly
        ...
        @else
            ...
            <div class="flex gap-2">
                <x-forms.input label="Username" id="cassandraAdminUser" required canGate="update" :canResource="$database" />
                @if ($isPasswordHiddenForMember)
                    <x-forms.input label="Password" disabled value="Hidden (only admins can view)" />
                @else
                    <x-forms.input label="Password" id="cassandraAdminPassword" type="password" required canGate="update"
        ...
        <div class="flex flex-col gap-2">
            ...
            <div class="flex items-end gap-2">
                <x-forms.input placeholder="9042:9042" id="portsMappings" label="Ports Mappings"
                    helper="A comma separated list of ports you would like to map to the host system.<br><span class='inline-block font-bold dark:text-warning'>Example</span>9042:9042,7000:7000"
                    canGate="update" :canResource="$database" />
            </div>
        </div>
        <livewire:project.database.cassandra.status-info :database="$database" />
            ...
            <div class="flex flex-col gap-2">
            <x-forms.input type="number" placeholder="9042" disabled="{{ $isPublic }}" id="publicPort" label="Public Port"
                canGate="update" :canResource="$database" />
