<div class="application-settings-form">
    <form wire:submit="submit" class="flex flex-col gap-6">
        ...
        <x-application.settings-section title="Database details"
            description="Manage the identity and container image for this Cassandra database.">
            <x-slot:actions>
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                <div class="lg:col-span-2">
                    <x-forms.input label="Image" id="image" required canGate="update" :canResource="$database"
                        helper="Use a published bitnami/cassandra image from Docker Hub." />
                </div>
            </div>
        </x-application.settings-section>

        <x-application.settings-section title="Credentials"
            description="Keep these values aligned with the credentials configured inside Cassandra.">
            ...
            <div class="{{ !$database->started_at ? 'mt-4 ' : '' }}grid gap-4 lg:grid-cols-2">
                <x-forms.input label="{{ $database->started_at ? 'Initial username' : 'Username' }}"
                    id="cassandraAdminUser" placeholder="If empty: cassandra"
                ...
                @else
                    <x-forms.input label="{{ $database->started_at ? 'Initial password' : 'Password' }}"
                        id="cassandraAdminPassword" type="password" required
        ...
        <x-application.settings-section title="Runtime and network"
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                <x-forms.input placeholder="9042:9042" id="portsMappings" label="Port mappings"
                    helper="Comma-separated host-to-container mappings, for example 9042:9042."
        ...
        <x-application.settings-section title="Public access" class="relative"
            ...
            <div class="grid gap-4 lg:grid-cols-2">
                ...
                <x-forms.input type="number" placeholder="9042" disabled="{{ $isPublic }}" id="publicPort"
