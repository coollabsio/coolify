<div wire:init='refreshBackupExecutions'>
    {{-- pgBackRest Restore Progress Modal --}}
    @if ($showRestoreProgress)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @if ($restoreStatus === 'running') wire:poll.2000ms="pollRestoreStatus" @endif>
            <div class="bg-neutral-900 rounded-lg shadow-xl w-full max-w-lg mx-4 overflow-hidden">
                <div class="flex items-center justify-between p-4 border-b border-neutral-700">
                    <h3 class="text-lg font-semibold flex items-center gap-2">
                        @if ($restoreStatus === 'running')
                            <svg class="animate-spin h-5 w-5 text-warning" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                                <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
                                <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
                            </svg>
                        @elseif ($restoreStatus === 'success')
                            <svg class="h-5 w-5 text-success" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                            </svg>
                        @elseif ($restoreStatus === 'failed')
                    ...
                    @if ($restoreStatus !== 'running')
                        <button wire:click="dismissRestoreProgress" class="text-neutral-400 hover:text-white">
                ...
                <div class="p-4">
                            ...
                            <div class="flex items-center gap-3">
                                <div class="flex-1">
                                    <div class="h-2 bg-neutral-700 rounded-full overflow-hidden">
                                        <div class="h-full bg-warning animate-pulse rounded-full" style="width: 100%"></div>
                                    </div>
                                </div>
                            </div>
                            @if ($restoreBackupLabel)
                                <p class="text-sm text-neutral-300">
                                    <span class="font-medium">Restoring from:</span>
                                    <code class="ml-1 px-2 py-0.5 bg-neutral-800 rounded text-xs">{{ $restoreBackupLabel }}</code>
                                </p>
                            @endif
                            <div class="p-3 bg-neutral-800 rounded text-sm">
                                <p class="text-neutral-400">{{ $restoreMessage ?? 'Restore in progress...' }}</p>
                            </div>
                            <div class="p-3 bg-blue-900/30 border border-blue-700/50 rounded text-sm">
                                <p class="text-blue-300">
                                    <strong>Note:</strong> The database is temporarily unavailable during restore.
                                    This modal will update automatically when the restore completes.
                                </p>
                            </div>
                        </div>
                    ...
                    @elseif ($restoreStatus === 'success')
                        <div class="space-y-4">
                            <div class="p-4 bg-green-900/30 border border-green-700/50 rounded">
                                <p class="text-green-300 font-medium">Restore completed successfully!</p>
                                <p class="text-green-400/80 text-sm mt-1">The database has been restored and restarted.</p>
                            </div>
                            <x-forms.button wire:click="dismissRestoreProgress" class="w-full">
                                Close
                            </x-forms.button>
                        </div>
                    ...
                    @elseif ($restoreStatus === 'failed')
                        <div class="space-y-4">
                            <div class="p-4 bg-red-900/30 border border-red-700/50 rounded">
                                <p class="text-red-300 font-medium">Restore failed</p>
                                @if ($restoreMessage)
                                    <div class="mt-2 p-2 bg-neutral-900 rounded text-sm font-mono text-red-400 max-h-48 overflow-y-auto whitespace-pre-wrap">{{ $restoreMessage }}</div>
                                @endif
                            </div>
                            <x-forms.button wire:click="dismissRestoreProgress" class="w-full">
                                Dismiss
