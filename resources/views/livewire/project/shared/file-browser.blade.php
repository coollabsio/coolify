                                ...
                                @foreach ($entries as $index => $entry)
                                    <tr class="border-b dark:border-neutral-800 hover:dark:bg-neutral-800/50">
                                        ...
                                        <td class="py-1.5 px-2">
                                            @if ($entry['isDirectory'])
                                                <button wire:click="navigateTo({{ $index }})" class="flex items-center gap-1 hover:underline dark:text-white font-medium">
                                        ...
                                        <td class="py-1.5 px-2 text-right whitespace-nowrap">
                                            @if ($entry['isDirectory'])
                                                <button wire:click="downloadFolder({{ $index }})" class="text-xs hover:underline dark:text-neutral-400 hover:dark:text-white" title="Download as .tar.gz">
                                                    Download
                                                </button>
                                                <span class="dark:text-neutral-600 px-1">|</span>
                                            @else
                                                <button wire:click="downloadFile({{ $index }})" class="text-xs hover:underline dark:text-neutral-400 hover:dark:text-white" title="Download file">
                                                    Download
                                                </button>
                                                <span class="dark:text-neutral-600 px-1">|</span>
                                            @endif
                                            <button
                                                ...
                                                x-on:click="if (confirm('Delete ' + entryName + '? This cannot be undone.')) { $wire.deleteEntry({{ $index }}) }"
