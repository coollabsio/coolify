<div>
    <x-slot:title>
        {{ data_get_str($application, 'name')->limit(10) }} > {{ __('deployment.title') }} | Coolify
        </x-slot>
        <h1 class="py-0">{{ __('deployment.title') }}</h1>
                ...
                <div @if ($isKeepAliveOn) wire:poll.2000ms="polling" @endif
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-3">
                            @if (data_get($application_deployment_queue, 'status') === 'in_progress')
                                <div class="flex items-center gap-1">
                                    <span>{{ __('deployment.status_label') }}</span>
                                    <span class="dark:text-warning">{{ __('deployment.status.in_progress') }}</span>
                            ...
                            @endif
                            <span x-show="searchQuery.trim()" x-text="matchCount + ' {{ __('shared.matches') }}'"
                        ...
                        <div class="flex items-center gap-2">
                            <div class="relative">
                                ...
                                <input type="text" x-model.debounce.300ms="searchQuery" placeholder="{{ __('shared.find_in_logs') }}"
                            ...
                            <button
                                x-on:click="
                                    $wire.copyLogs().then(logs => {
                                        ...
                                        Livewire.dispatch('success', ['{{ __('shared.logs_copied') }}']);
                                    });
                                "
                                title="{{ __('shared.copy_logs') }}"
                            ...
                            <button x-on:click="downloadLogs()" title="{{ __('shared.download_logs') }}"
                                class="p-1 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                                <svg class="w-4 h-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                    stroke-width="1.5" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round"
                            ...
                            <button title="{{ __('shared.toggle_timestamps') }}" x-on:click="showTimestamps = !showTimestamps"
                            ...
                            <button wire:click="toggleDebug"
                                title="{{ $is_debug_enabled ? __('shared.hide_debug_logs') : __('shared.show_debug_logs') }}"
                            ...
                            <button title="{{ __('shared.follow_logs') }}" :class="alwaysScroll ? '!text-warning' : ''"
                            ...
                            <button title="{{ __('shared.fullscreen') }}" x-show="!fullscreen" x-on:click="makeFullscreen"
                                class="p-1 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                                <svg class="w-4 h-4" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <g fill="none">
                                        <path
                            ...
                            <button title="{{ __('shared.minimize') }}" x-show="fullscreen" x-on:click="makeFullscreen"
                                class="p-1 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200">
                                <svg class="w-4 h-4" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                    <path fill="none" stroke="currentColor" stroke-linecap="round"
                    ...
                    <div id="logsContainer"
                        ...
                        <div id="logs" class="flex flex-col font-mono">
                            ...
                            @empty
                                <span class="font-mono text-neutral-400 mb-2">{{ __('shared.no_logs_yet') }}</span>
                            @endforelse
                        </div>
                    </div>
                </div>
