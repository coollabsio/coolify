<div>
    @if (subscriptionProvider() === 'stripe')
        <div class="pt-4">
            <h2>{{ __('subscription.current_plan') }}</h2>
            <div class="pb-4">{{ __('subscription.tier') }} <strong class="dark:text-warning">
                    @if (data_get(currentTeam(), 'subscription')->type() == 'dynamic')
                        {{ __('subscription.pay_as_you_go') }}
                    @else
            ...
            @if (currentTeam()->subscription->stripe_cancel_at_period_end)
                <div class="pb-2">{{ __('subscription.active_cancel_period') }}</div>
            @else
                <div class="pb-2">{{ __('subscription.active_last_invoice') }}
                    {{ currentTeam()->subscription->stripe_invoice_paid ? __('subscription.paid') : __('subscription.not_paid') }}.</div>
            @endif
            <div class="flex items-center gap-2">
                <div class="w-48">{{ __('subscription.paid_servers_count') }}</div>
                <div class="text-xl font-bold dark:text-white">{{ $server_limits }}</div>
            </div>
            <div class="flex items-center gap-2">
                <div class="w-48">{{ __('subscription.active_servers_count') }}</div>
                <div class="text-xl font-bold dark:text-white">{{ currentTeam()->servers->count() }}</div>
            </div>
            @if (currentTeam()->serverOverflow())
                <x-callout type="danger" title="{{ __('warning.title') }}" class="my-4">
                    {{ __('subscription.server_overflow_warning', ['count' => currentTeam()->servers->count() - $server_limits]) }}
                </x-callout>
            @endif
            <x-forms.button class="gap-2" wire:click='stripeCustomerPortal'>{{ __('subscription.change_server_quantity') }}
            </x-forms.button>
            <h2 class="pt-4">{{ __('subscription.manage_subscription') }}</h2>
            <div class="pb-4">{{ __('subscription.manage_subscription_desc') }}</div>
            <div class="flex gap-2">
                <x-forms.button class="gap-2" wire:click='stripeCustomerPortal'>{{ __('subscription.go_to') }} <svg
        ...
        <div class="pt-4">
            {{ __('subscription.help_text') }} <a class="underline dark:text-white" href="{{ config('constants.urls.contact') }}"
                target="_blank">{{ __('subscription.contact_us') }}</a>
        </div>
    @endif
</div>
