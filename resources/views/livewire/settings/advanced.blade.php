                ...
                <div class="flex flex-col gap-1">
                    ...
                    <div class="md:w-96">
                        <x-forms.checkbox instantSave id="is_api_enabled" label="API Access"
                            helper="If enabled, authenticated requests to Coolify's REST API will be allowed. Configure API tokens in Keys & Tokens &raquo; API Tokens." />
                    </div>
                    ...
                    <div class="md:w-96">
                        <x-forms.checkbox instantSave id="is_mcp_server_enabled" label="Enable MCP Server"
                            helper="Exposes a Streamable HTTP Model Context Protocol endpoint at /mcp for AI clients (Claude Desktop, Cursor, etc.). Authenticates via Sanctum API tokens (Keys & Tokens &raquo; API Tokens). Requires API Access to be enabled." />
                    </div>
                    @if ($is_mcp_server_enabled)
                        <x-callout type="info" title="MCP Endpoint" class="mt-2">
                            Endpoint: <code>{{ url('/mcp') }}</code><br>
                            Authenticate with <code>Authorization: Bearer &lt;token&gt;</code> using a token created in Keys & Tokens &raquo; API Tokens.
                        </x-callout>
                    @endif
