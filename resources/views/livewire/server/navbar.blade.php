<div class="pb-6">
    ...
    <div class="navbar-main">
        <nav
            ...
            <a wire:navigate.hover class="{{ request()->routeIs('server.show') ? 'dark:text-white' : '' }}" href="{{ route('server.show', [
    'server_uuid' => data_get($server, 'uuid'),
            ...
            @if (!$server->isSwarmWorker() && !$server->settings->is_build_server)
                        <a wire:navigate.hover class="{{ request()->routeIs('server.proxy') ? 'dark:text-white' : '' }}" href="{{ route('server.proxy', [
                    'server_uuid' => data_get($server, 'uuid'),
                ]) }}">
                            Proxy
                        </a>
            @endif
            <a wire:navigate.hover class="{{ request()->routeIs('server.resources') ? 'dark:text-white' : '' }}" href="{{ route('server.resources', [
    'server_uuid' => data_get($server, 'uuid'),
]) }}">
                Resources
            </a>
            @can('canAccessTerminal')
                        <a wire:navigate.hover class="{{ request()->routeIs('server.command') ? 'dark:text-white' : '' }}" href="{{ route('server.command', [
                    'server_uuid' => data_get($server, 'uuid'),
                ]) }}">
                            Terminal
                        </a>
            @endcan
            @can('update', $server)
                        <a wire:navigate.hover class="{{ request()->routeIs('server.security.patches') ? 'dark:text-white' : '' }}"
