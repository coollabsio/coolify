...
<div>
    @if (
        ...
        <div wire:poll="updateProxyStatus" x-data="{ proxyStatus: @entangle('proxyStatus').live }" x-init="$watch('proxyStatus', value => { console.log('Proxy status changed:', value); $wire.$refresh(); })">
            <div class="flex gap-2">
                @if($server->proxyType() !== 'NONE' && $server->proxy && $server->proxy->type)
                    <template x-if="proxyStatus === 'Proxy Running'">
                        @if (
                    ...
                    <template x-if="proxyStatus === 'Proxy Stopped' || proxyStatus === 'Proxy Exited'">
                    ...
                    <template x-if="proxyStatus === 'Proxy Restarting'">
                        <div class="flex items-center gap-2">
                            <x-loading />
                            <span>Proxy is restarting...</span>
                        </div>
                    </template>
                @endif
            </div>
