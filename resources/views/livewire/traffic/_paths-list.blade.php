{{--
    Paginated "top paths" list. Each row shows the owning domain (when known) above
    the path, and links the path to its live URL (new tab, rel="noopener noreferrer
            ...
            <div wire:key="{{ $keyPrefix }}-{{ $loop->index }}"
                ...
                @if ($href)
                    <a href="{{ $href }}" target="_blank" rel="noopener noreferrer nofollow"
                        class="min-w-0 flex-1 truncate font-mono text-[12px] text-black hover:underline dark:text-fg">@if ($domain)<span class="text-neutral-400 dark:text-fg-faint">{{ $domain }}</span>@endif{{ $pathStr }}</a>
                @else
                    <span class="min-w-0 flex-1 truncate font-mono text-[12px] text-black dark:text-fg">@if ($domain)<span class="text-neutral-400 dark:text-fg-faint">{{ $domain }}</span>@endif{{ $pathStr }}</span>
                @endif
