<div class="flex flex-col items-start gap-2 min-w-fit">
    ...
    @if ($server->hetzner_server_id)
        <a wire:navigate class="menu-item {{ $activeMenu === 'cloud-provider-token' ? 'menu-item-active' : '' }}"
            href="{{ route('server.cloud-provider-token', ['server_uuid' => $server->uuid]) }}">Hetzner Token
        </a>
    @endif
    <a wire:navigate class="menu-item {{ $activeMenu === 'ca-certificate' ? 'menu-item-active' : '' }}"
        href="{{ route('server.ca-certificate', ['server_uuid' => $server->uuid]) }}">CA Certificate
    </a>
    @if (!$server->isLocalhost())
        <a wire:navigate class="menu-item {{ $activeMenu === 'cloudflare-tunnel' ? 'menu-item-active' : '' }}"
