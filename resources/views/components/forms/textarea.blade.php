        ...
        @if ($type === 'password')
            <div class="relative" x-data="{ type: 'password' }">
                ...
                <textarea minlength="{{ $minlength }}" maxlength="{{ $maxlength }}" x-cloak x-show="type !== 'password'"
                    ...
                    @if ($realtimeValidation) wire:model.debounce.200ms="{{ $modelBinding }}" wire:dirty.class="ring-2 ring-coollabs dark:ring-warning"
                @else
            wire:model={{ $value ?? $modelBinding }} wire:dirty.class="ring-2 ring-coollabs dark:ring-warning" @endif
