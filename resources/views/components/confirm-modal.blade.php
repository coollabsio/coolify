<script>
    document.addEventListener('alpine:init', () => {
        Alpine.data('confirmModal', () => ({
            open: false,
            confirmAction: null,
            message: '{{ __('modal.confirm.title') }}',
...
<div x-cloak x-show="open" x-transition class="fixed inset-0 z-50 flex pt-10">
    <div @click.away="open = false" class="w-screen h-20 max-w-xl mx-auto bg-black rounded-lg">
        <div class="flex flex-col items-center justify-center h-full">
            ...
            <div>
                <x-forms.button x-on:click='confirmed()'>{{ __('modal.buttons.confirm') }}</x-forms.button>
                <x-forms.button x-on:click="open = false">{{ __('modal.buttons.cancel') }}</x-forms.button>
            </div>
        </div>
    </div>
</div>
