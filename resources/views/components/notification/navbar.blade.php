<div class="pb-6">
    ...
    <div class="navbar-main">
        <nav class="flex items-center gap-6 min-h-10">
            ...
            <a wire:navigate class="{{ request()->routeIs('notifications.slack') ? 'dark:text-white' : '' }}"
                href="{{ route('notifications.slack') }}">
                <button>Slack</button>
            </a>
            <a wire:navigate class="{{ request()->routeIs('notifications.pushover') ? 'dark:text-white' : '' }}"
                href="{{ route('notifications.pushover') }}">
                <button>Pushover</button>
            </a>
            <a wire:navigate class="{{ request()->routeIs('notifications.webhook') ? 'dark:text-white' : '' }}"
