...
{{--
    Small glyph shown on a resource node / list row, matching Railway's iconography:
    - git/github  -> GitHub mark (repo-backed application)
    - terminal    -> `>_` (docker-image app, command-style resource)
    - database    -> database cylinder
    - service     -> stacked squares (compose/service stack)
--}}
@php
    $t = strtolower($type);
@endphp

<span {{ $attributes->merge(['class' => 'inline-flex items-center justify-center text-rw-text']) }}>
    @switch($t)
        @case('github')
        @case('git')
            <svg class="{{ $size }}" viewBox="0 0 16 16" fill="currentColor" aria-hidden="true">
        ...
        @case('command')
            <svg class="{{ $size }}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
                <path d="m5 8 4 4-4 4"/><path d="M12 16h6"/>
            </svg>
            @break
        @case('database')
            <svg class="{{ $size }}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
