...
it('shows explicit loading spinners and disables cloud token action buttons while requests run', function () {
    ...
    expect(substr_count($tokenFormView, 'wire:loading.attr="disabled" wire:target="addToken"'))->toBe(1)
        ->and(substr_count($tokenFormView, '<x-loading-on-button wire:loading wire:target="addToken" />'))->toBe(1)
