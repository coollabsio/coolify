...
it('creates standalone destinations with an IPv6-enabled network and falls back to IPv4-only', function () {
    ...
    $safe = escapeshellarg('test-network');

    expect($model->networkCreateCommand())
        ->toBe("docker network inspect {$safe} >/dev/null 2>&1 || docker network create --attachable --ipv6 {$safe} >/dev/null 2>&1 || docker network create --attachable {$safe} >/dev/null");
});
