...
it('shows a warning icon when a server menu item requires attention', function () {
    ...
    expect($contents)
        ...
        ->toContain('proxyNotRunning: @js($proxyNotRunning)')
        ->toContain('proxyNotRunning = $event.detail.proxyNotRunning')
        ->toContain('x-show="proxyConfigurationPending || traefikOutdated || proxyNotRunning"')
