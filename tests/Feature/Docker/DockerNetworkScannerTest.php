...
it('removes missing inventory rows and remains idempotent', function () {
    $server = $this->createFunctionalServer();
    ...
    expect($first->get('removed'))->toBe(1)
        ->and($second->get('created'))->toBe(0)
        ->and(DockerNetwork::where('docker_network_name', 'missing-net')->exists())->toBeFalse();
});
