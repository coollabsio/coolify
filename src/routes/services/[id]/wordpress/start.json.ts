...
export const post: RequestHandler<Locals> = async (event) => {
    ...
    try {
        ...
        const { type, version, fqdn, destinationDockerId, destinationDocker, wordpress: { mysqlDatabase, mysqlUser, mysqlPassword, extraConfig, mysqlRootUser, mysqlRootUserPassword } } = service
        ...
        try {
            await asyncExecShell(`DOCKER_HOST=${host} docker volume create ${config.mysql.volume.split(':')[0]}`)
            await asyncExecShell(`DOCKER_HOST=${host} docker volume create ${config.wordpress.volume.split(':')[0]}`)
        } catch (error) {
            console.log(error)
        }
