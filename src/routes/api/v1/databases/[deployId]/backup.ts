...
export async function post(request: Request) {
    ...
    try {
        const now = new Date()
        const configuration = JSON.parse(JSON.parse(await execShellAsync(`docker inspect ${deployId}_${deployId}`))[0].Spec.Labels.configuration)
        const type = configuration.general.type
        const serviceId = configuration.general.deployId
        const databaseService = (await docker.engine.listContainers()).find(r => r.Labels['com.docker.stack.namespace'] === serviceId && r.State === 'running')
        ...
        if (type === 'mongodb') {
            if (databaseService) {
                ...
                await execShellAsync(`docker exec -i ${containerID} /bin/bash -c "mkdir -p ${tmpdir};mongodump --uri='mongodb://${username}:${password}@${deployId}:27017' -d ${databaseName} --gzip --archive=${fullfilename}"`)
            ...
            if (databaseService) {
                ...
                const fullfilename = `${tmpdir}/${filename}`
                await execShellAsync(`docker exec -i ${containerID} /bin/bash -c "PGPASSWORD=${password} pg_dump --username ${username} -Z 9 ${databaseName}" > ${fullfilename}`)
                return {
                    status: 200,
                    headers: {
                        'Content-Type': 'application/octet-stream',
            ...
            if (databaseService) {
                ...
                const fullfilename = `${tmpdir}/${filename}`
                await execShellAsync(`docker exec -i ${containerID} /bin/bash -c "cd /bitnami/couchdb/data/ && tar -czvf - ." > ${fullfilename}`)
                return {
                    status: 200,
                    headers: {
                        'Content-Type': 'application/octet-stream',
            ...
            if (databaseService) {
                ...
                const filename = `${databaseName}_${now.getTime()}.sql.gz`
                const fullfilename = `${tmpdir}/${filename}`
                await execShellAsync(`docker exec -i ${containerID} /bin/bash -c "mysqldump -u ${username} -p${password} ${databaseName} | gzip -9 -" > ${fullfilename}`)
                return {
                    status: 200,
                    headers: {
                        'Content-Type': 'application/octet-stream',
