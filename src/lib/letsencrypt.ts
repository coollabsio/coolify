...
export async function letsEncrypt(data) {
    const { destinationDocker, domain, forceSSLChanged, isCoolify = false, id } = data
    try {
        if (dev) {
            if (forceSSLChanged) {
                return await forceSSLOnApplication({ domain })
            } else {
                return await forceSSLOffApplication({ domain })
            }
        } else {
            if (isCoolify) {
