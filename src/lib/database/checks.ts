...
export async function isBranchAlreadyUsed({ repository, branch, id }) {
    ...
    return await prisma.application.findFirst({ where: { branch, repository, gitSource: { type: application.gitSource.type } } })
}
...
export async function isDomainConfigured({ id, fqdn }) {
    ...
    const foundApp = await prisma.application.findFirst({ where: { fqdn: { endsWith: domain }, id: { not: id } }, select: { fqdn: true } })
    const foundService = await prisma.service.findFirst({ where: { fqdn: { endsWith: domain }, id: { not: id } }, select: { fqdn: true } })
