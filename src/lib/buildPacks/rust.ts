...
const createDockerfile = async (data, image, name): Promise<void> => {
    ...
    Dockerfile.push('WORKDIR /usr/src/app')
    Dockerfile.push(`RUN apt-get update -y && apt-get install -y --no-install-recommends openssl libcurl4 ca-certificates && apt-get autoremove -y && apt-get clean -y && rm -rf /var/lib/apt/lists/*`)
    Dockerfile.push(`RUN update-ca-certificates`)
    Dockerfile.push(`COPY --from=${applicationId}:${tag}-cache /usr/src/app/target/release/${name} ${name}`)
