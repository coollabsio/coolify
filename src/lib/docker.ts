...
export async function buildImage({ applicationId, commit, workdir, docker, buildId, isCache = false }) {
    const stream = await docker.engine.buildImage(
        { src: ['.'], context: workdir },
        { dockerfile: isCache ? 'Dockerfile-cache' : 'Dockerfile', t: `${applicationId}:${commit.slice(0, 7)}${isCache ? '-cache' : ''}` }
    );
    await streamEvents({ stream, docker, buildId, applicationId })
}
...
export async function streamEvents({ stream, docker, buildId, applicationId }) {
