<script>
  /// Accessing  http://localhost:3000/application/coollabsio/coollabs.io/main/logs errors!!!!!!!
  ...
  async function loadDeployments() {
    if ($configuration.repository.id) {
      deployments = await $fetch(`/api/v1/application/logs?repoId=${$configuration.repository.id}`);
